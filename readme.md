...
# serial - обмен данными с ардуиной

Пришло время разобраться как общаться с Arduino. Не перезаливать же скетч каждый раз когда надо сменить пароль или установить какие то параметры. Самый простой вариант передача данных через серийный порт. Используя серийный порт Arduino может как посылать данные так и получать. Для того чтобы увидеть как это работает в комплект Arduino IDE входит монитор порта. В теории можно воспользоваться другими терминальными программами.

Маленький лайв хак - ATmega32U4 не успевает инициализировать связь по серийному порту и может пропустить пару строк. Для того чтобы этого избежать делаем задержку до момента пока не инициализируется серийный порт.
...
Хотя при таком подходе возникает другая проблема, код зависнет до момента подключения терминала. Иначе говоря, ардуина не запустится и не будет работать без подключенного терминала. Как показала практика, Serial примет TRUE при первом подключении терминала и будет оставаться таковым до выключения, либо возможно до Serial.end, но так далеко я не заходил =) по той причине, что ардуина продолжает принимать значения после переподключения терминала, а спамить Serial.begin каждую итерацию цикла loop я посчитал излишним. Можно, конечно, придумать какой то таймер и обновлять соединение один раз в секунду, но это все фантазии. В общем пока и так сойдет =)

Пример: [serial](/serial)
...
Подключение MFRC-522 не требует дополнительных деталей (резисторы, конденсаторы и т.п.). Используется стандарт SDI, в [git проекта](https://github.com/miguelbalboa/rfid) есть табличка где расписано, что к каким пинам надо подключать. Замечу, что таблица с главной страницы гита и в комментариях к примерам разнится, используем ту что на главной.

![rfid_bb](/images/rfid_bb.png)

Пины расписаны не все, думаю автор посчитал, что вам будет не сложно догадаться куда должны идти GND и 3.3V. IRQ не идет некуда и об этом нигде не сказано, либо я не нашел.
...
Четвертый блок сектора хранит информацию о правах доступа - два ключа A и B и биты доступа которые описывают права доступа к данным для ключей. Обычно этот блок называют trailer.

Для карт объемом выше 4Кб сектора за переделами отметки 2Кб имеют размер 256 байт разделенные на 16 блоков где trailer - шестнадцатый блок.
...
Сектор 1 может использоваться для хранения CIS индексов, что это такое я не понял, но думаю, что трогать сектор 1 не стоит, хотя... как показала практика трогать можно, да и в примерах в него что-то пишут.

Стоит обратить внимание на тот факт, что нумерация блоков не зависит от номера сектора, то есть нулевой блок сектора 1 будет равен 4.

Блок доступа trailer имеет следующий формат
...
|---------|------------------------------------------|---------|
| 0..5    | ключ A                                   | 6 байт  |
| 6..8    | биты доступа (access bits)               | 3 байта |
| 9       | не используется (данные пользователя)    | 1 байт  |
| 10 - 15 | ключ B                                   | 6 байт  |

Биты доступа формируются по сложной и запутанной схеме. Подробно о порядке формирования можно узнать в документации на 11 странице, но лучше воспользоваться калькулятором.

[Документация RFID](https://www.nxp.com/docs/en/data-sheet/MF1S50YYX_V1.pdf)

[Калькулятор бит доступа](http://calc.gmss.ru/Mifare1k/)

Стандартное значение трейлера

```
const char defaultTrailerData[16] = {
...
При получении дампа блока трейлера значение ключа A состоят из нулей, это не так, просто ключ A прочесть нельзя. На самом деле там по умолчанию все 0xFF. Ключ B по умолчанию вообще не активен, и может быть использован для хранения пользовательских данных.

Примеры:

[rfid_rw](/rfid_rw) - Чтение и запись данных

[rfid_rw_trailer](/rfid_rw_trailer) - Чтение и запись данных а также запись в блок трейлера стандартного ключа. То что этот код НЕ ЗАПАРЫВАЕТ блок и есть успех его работы =)

[rfid_password](/rfid_password) - Хранит пароль длинной до 16 символов на метке, при поднесении печатает его как клавиатура, если ввести строку пароля в консоли переходит в режим записи пароля и ждет карту для записи. Любой другой ввод в режиме записи пароля выходит из режима. Для хранения пароля использует первый сектор, изменяет ключ A.
